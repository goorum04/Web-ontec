...
const HOTSPOTS = [
  ...
  },
  {
    ...
    specs: [
      ...
      { ca: 'Panell tàctil Crestron TSW-1070 de 10" muntat en taula', es: 'Panel táctil Crestron TSW-1070 de 10" montado en mesa', fr: 'Panneau tactile Crestron TSW-1070 de 10" monté en table', en: '10" Crestron TSW-1070 touch panel mounted on table' },
              ...
              return (
                <div key={h.id} style={{ background: 'rgba(255,255,255,.05)', border: `1px solid ${h.color}44`, borderRadius: 16, padding: '20px 20px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
                    <span style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: `${h.color}22`, border: `1px solid ${h.color}55`, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Ico s={22} /></span>
                    <div>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: h.color, textTransform: 'uppercase', letterSpacing: '.14em' }}>{h.n} · {tt(h.label)}</div>
                      <div style={{ fontFamily: 'var(--disp)', fontWeight: 800, fontSize: 17, color: '#fff' }}>{tt(h.title)}</div>
                    </div>
                  </div>
                  <p style={{ fontSize: 13, color: 'rgba(255,255,255,.65)', lineHeight: 1.7, marginBottom: 14 }}>{tt(h.desc)}</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {h.specs.map((s, i) => (
                      <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                        <span style={{ color: h.color, marginTop: 1, flexShrink: 0, display: 'inline-flex' }}><Icons.Check /></span>
                        <span style={{ fontSize: 12.5, color: 'rgba(255,255,255,.75)', lineHeight: 1.5 }}>{tt(s)}</span>
                      </div>
                    ))}
                  </div>
                </div>
